...
const BananaChatbot = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome-msg",
      sender: "bot",
      text: "สวัสดีครับ ผมคือ Banana Assistant 🍌\nยินดีตอบทุกคำถามเรื่องสายพันธุ์กล้วย การปลูก การดูแลรักษา และการแก้ไขโรคพืชครับ พิมพ์คำถามเข้ามาได้เลยครับ!",
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end">
      {/* 🟢 Chat Window */}
      {isOpen && (
        <Card className="w-[92vw] sm:w-[400px] h-[520px] max-h-[80vh] flex flex-col mb-3 shadow-2xl rounded-2xl border border-slate-200/80 bg-white/95 backdrop-blur-md overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          {/* Header */}
          <div className="bg-gradient-to-r from-yellow-500 via-orange-500 to-amber-600 text-white px-4 py-3.5 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-lg">
                🍌
              </div>
              <div>
                <h4 className="font-bold text-sm leading-tight text-white">Banana Assistant</h4>
          ...
          <ScrollArea className="flex-1 p-4 bg-slate-50/60">
            <div className="space-y-3.5">
              {messages.map((msg) => (
                ...
                >
                  {msg.sender === "bot" && (
                    <div className="w-7 h-7 rounded-full bg-orange-100 border border-orange-200 flex items-center justify-center flex-shrink-0 text-xs">
                      🍌
                    </div>
                  )}
              ...
              {isTyping && (
                <div className="flex gap-2.5 items-center">
                  ...
                  <div className="bg-white border border-slate-200 px-4 py-2.5 rounded-2xl rounded-bl-xs shadow-sm flex items-center gap-1.5">
          ...
          <div className="p-3 bg-white border-t border-slate-100 flex items-center gap-2">
            <Input
              ...
              className="flex-1 rounded-xl border-slate-200 focus-visible:ring-orange-500 text-sm py-2"
            />
            <Button
              onClick={handleSend}
              disabled={!input.trim() || isTyping}
              className="bg-orange-600 hover:bg-orange-700 text-white rounded-xl h-10 w-10 p-0 flex items-center justify-center shadow-md transition-transform active:scale-95 disabled:opacity-50"
            >
      ...
      <Button
        ...
        className="h-14 w-14 rounded-full bg-gradient-to-r from-yellow-500 to-orange-600 text-white shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-200 flex items-center justify-center p-0 group"
